...
function exibirMensagemParabens() {
  // Exibir mensagem de parabéns a cada 300 pontos
  ...
  // Exibir mensagem especial do Enzo a cada 500 pontos
  if (pontuacao % 500 === 0 && pontuacao > 0) {
    const enzoMessage = document.createElement("div");
    enzoMessage.id = "enzo-message";
    enzoMessage.innerHTML = "Eu ENZO consigo fácil mais pontos, tente mim superar, ah, ah, ah...";
    document.body.appendChild(enzoMessage);

    // Estilos para o pop-up do Enzo
    enzoMessage.style.position = "fixed";
    enzoMessage.style.top = "50%";
    enzoMessage.style.left = "50%";
    enzoMessage.style.transform = "translate(-50%, -50%)";
    enzoMessage.style.backgroundColor = "rgba(0, 0, 0, 0.8)";
    enzoMessage.style.color = "#ffcc00";
    enzoMessage.style.padding = "20px";
    enzoMessage.style.borderRadius = "10px";
    enzoMessage.style.fontSize = "1.5em";
    enzoMessage.style.textAlign = "center";
    enzoMessage.style.boxShadow = "0px 0px 15px rgba(255, 204, 0, 0.5)";
    enzoMessage.style.zIndex = "1000";

    // Remover o pop-up após 5 segundos
    setTimeout(() => {
      enzoMessage.remove();
    }, 5000);
  }
...
/* Ajuste para centralizar e escalar o jogo */
...
function adjustGameLayout() {
  ...
  // Calcula o fator de escala com base no tamanho da tela
  const scale = Math.min(
    window.innerWidth / gameContainer.offsetWidth,
    window.innerHeight / gameContainer.offsetHeight
  );

  // Aplica a escala e centraliza
  gameContainer.style.transform = `scale(${scale})`;
  gameContainer.style.transformOrigin = 'top left';
  gameContainer.style.position = 'absolute';
  gameContainer.style.top = `${(window.innerHeight - gameContainer.offsetHeight * scale) / 2}px`;
  gameContainer.style.left = `${(window.innerWidth - gameContainer.offsetWidth * scale) / 2}px`;
}
